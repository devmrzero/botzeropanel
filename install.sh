...
function show_menu() {
    show_logo
    echo -e "\033[1;36m1)\033[0m Install zero Bot"
    echo -e "\033[1;36m2)\033[0m Update zero Bot"
    echo -e "\033[1;36m3)\033[0m Remove zero Bot"
...
function install_bot() {
    echo -e "\e[32mInstalling zero script ... \033[0m\n"
    ...
    echo 'phpmyadmin phpmyadmin/app-password-confirm password zerohipass' | sudo debconf-set-selections
    echo 'phpmyadmin phpmyadmin/mysql/admin-pass password zerohipass' | sudo debconf-set-selections
    echo 'phpmyadmin phpmyadmin/mysql/app-pass password zerohipass' | sudo debconf-set-selections
    ...
    BOT_DIR="/var/www/html/zerobotconfig"
    if [ -d "$BOT_DIR" ]; then
    ...
    ZIP_URL=$(curl -s https://api.github.com/repos/devmrzero/botzeropanel/releases/latest | grep "zipball_url" | cut -d '"' -f 4)

# Check for version flag
if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
    ZIP_URL="https://github.com/devmrzero/botzeropanel/archive/refs/heads/main.zip"
elif [[ "$1" == "-v" && -n "$2" ]]; then
    ZIP_URL="https://github.com/devmrzero/botzeropanel/archive/refs/tags/$2.zip"
fi

    # Download and extract the repository
    TEMP_DIR="/tmp/zerobot"
    mkdir -p "$TEMP_DIR"
    ...
    echo -e "\n\033[33mzero config and script have been installed successfully.\033[0m"


wait
if [ ! -d "/root/confzero" ]; then
    sudo mkdir /root/confzero || {
        echo -e "\e[91mError: Failed to create /root/confzero directory.\033[0m"
        exit 1
    }

    sleep 1

    touch /root/confzero/dbrootzero.txt || {
        echo -e "\e[91mError: Failed to create dbrootzero.txt.\033[0m"
        exit 1
    }
    sudo chmod -R 777 /root/confzero/dbrootzero.txt || {
        echo -e "\e[91mError: Failed to set permissions for dbrootzero.txt.\033[0m"
        exit 1
    }
    sleep 1
    ...
    echo "${ASAS}user = 'root';" >> /root/confzero/dbrootzero.txt
    echo "${ASAS}pass = '${randomdbpasstxt}';" >> /root/confzero/dbrootzero.txt
    echo "${ASAS}path = '${RANDOM_NUMBER}';" >> /root/confzero/dbrootzero.txt

    sleep 1

    passs=$(cat /root/confzero/dbrootzero.txt | grep '$pass' | cut -d"'" -f2)
    userrr=$(cat /root/confzero/dbrootzero.txt | grep '$user' | cut -d"'" -f2)
...
while [[ ! "$domainname" =~ ^[a-zA-Z0-9.-]+$ ]]; do
    ...
    PATHS=$(cat /root/confzero/dbrootzero.txt | grep '$path' | cut -d"'" -f2)
    sudo ufw allow 80 || {
    ...
    ROOT_PASSWORD=$(cat /root/confzero/dbrootzero.txt | grep '$pass' | cut -d"'" -f2)
    ROOT_USER="root"
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'zerobot'") ]]; then
            clear
            ...
            file_path="/var/www/html/zerobotconfig/config.php"
            ...
            echo -e "<?php" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}APIKEY = '${YOUR_BOT_TOKEN}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}usernamedb = '${dbuser}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}passworddb = '${dbpass}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}dbname = '${dbname}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}domainhosts = '${YOUR_DOMAIN}/zerobotconfig';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}adminnumber = '${YOUR_CHAT_ID}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}usernamebot = '${YOUR_BOTNAME}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}secrettoken = '${secrettoken}';" >> /var/www/html/zerobotconfig/config.php
            echo -e "${ASAS}connect = mysqli_connect('localhost', \$usernamedb, \$passworddb, \$dbname);" >> /var/www/html/zerobotconfig/config.php
            echo -e "if (${ASAS}connect->connect_error) {" >> /var/www/html/zerobotconfig/config.php
            echo -e "die(' The connection to the database failed:' . ${ASAS}connect->connect_error);" >> /var/www/html/zerobotconfig/config.php
            echo -e "}" >> /var/www/html/zerobotconfig/config.php
            echo -e "mysqli_set_charset(${ASAS}connect, 'utf8mb4');" >> /var/www/html/zerobotconfig/config.php
...
echo -e "$text_to_save" >> /var/www/html/zerobotconfig/config.php
            echo -e "?>" >> /var/www/html/zerobotconfig/config.php

            sleep 1

            curl -F "url=https://${YOUR_DOMAIN}/zerobotconfig/index.php" \
     ...
     "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook" || {
            ...
            url="https://${YOUR_DOMAIN}/zerobotconfig/table.php"
            curl $url || {
...
function install_bot_with_marzban() {
    ...
    read -p "Are you sure you want to install zero Bot alongside Marzban? (y/n): " confirm
    ...
    clear
    echo -e "\e[33mConfiguring zero Bot database credentials...\033[0m"
...
${ASAS}dbname = '$dbname';
${ASAS}domainhosts = '$YOUR_DOMAIN/zerobotconfig';
...
EOF
    ...
    for i in {1..5}; do
        curl "https://${YOUR_DOMAIN}/zerobotconfig/table.php" && break || {
...
function update_bot() {
    ...
    BOT_DIR="/var/www/html/zerobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[91mError: zero Bot is not installed. Please install it first.\033[0m"
        exit 1
    fi
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s https://api.github.com/repos/devmrzero/botzeropanel/releases/latest | grep "zipball_url" | cut -d '"' -f4)
    fi
    
    # Create temporary directory
    TEMP_DIR="/tmp/zerobot_update"
    mkdir -p "$TEMP_DIR"
    ...
    # Backup config file
    CONFIG_PATH="/var/www/html/zerobotconfig/config.php"
    TEMP_CONFIG="/root/zero_config_backup.php"
    ...
    # Remove old version
    sudo rm -rf /var/www/html/zerobotconfig || {
        echo -e "\e[91mFailed to remove old bot files!\033[0m"
        exit 1
    }
    
    # Move new files
    sudo mkdir -p /var/www/html/zerobotconfig
    sudo mv "$EXTRACTED_DIR"/* /var/www/html/zerobotconfig/ || {
    ...
    # Set permissions
    sudo chown -R www-data:www-data /var/www/html/zerobotconfig/
    sudo chmod -R 755 /var/www/html/zerobotconfig/
    ...
    echo -e "\n\e[92mzero Bot updated to latest version successfully!\033[0m"
}

# Delete Function
function remove_bot() {
    echo -e "\e[33mStarting zero Bot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"

    # Check if zero Bot is installed
    BOT_DIR="/var/www/html/zerobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m zero Bot is not installed (/var/www/html/zerobotconfig not found)." | tee -a "$LOG_FILE"
    ...
    # User Confirmation
    read -p "Are you sure you want to remove zero Bot and its dependencies? (y/n): " choice
    if [[ "$choice" != "y" ]]; then
    ...
    echo "Removing zero Bot..." | tee -a "$LOG_FILE"
    ...
    echo -e "\e[92mzero Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}

function remove_bot_with_marzban() {
    echo -e "\e[33mRemoving zero Bot alongside Marzban...\033[0m" | tee -a "$LOG_FILE"
    ...
    if [ ! -d "$BOT_DIR" ]; then
        ...
        DB_NAME="zerobot"  # Fallback to default database name
        DB_USER=""
    else
        ...
        if [ -f "$CONFIG_PATH" ]; then
            ...
            if [ -z "$DB_USER" ] || [ -z "$DB_NAME" ]; then
                ...
                DB_NAME="zerobot"  # Fallback to default
                DB_USER=""
            else
                echo -e "\e[92mFound database credentials: User=$DB_USER, Database=$DB_NAME\033[0m" | tee -a "$LOG_FILE"
            fi
        else
            echo -e "\e[93mWarning: config.php not found at $CONFIG_PATH. Assuming default database name 'zerobot'.\033[0m" | tee -a "$LOG_FILE"
            DB_NAME="zerobot"
            DB_USER=""
        fi
    ...
    if [ -n "$DB_USER" ]; then
        ...
        zero_USERS=$(docker exec "$MYSQL_CONTAINER" bash -c "mysql -u '$ROOT_USER' -p'$MYSQL_ROOT_PASSWORD' -e \"SELECT User FROM mysql.user WHERE User NOT IN ('root', 'mysql.infoschema', 'mysql.session', 'mysql.sys', 'marzban');\"" | grep -v "User" | awk '{print $1}')
        if [ -n "$zero_USERS" ]; then
            for user in $zero_USERS; do
    ...
    echo -e "\e[92mzero Bot has been removed alongside Marzban. SSL certificates remain intact.\033[0m" | tee -a "$LOG_FILE"
}
...
function auto_backup() {
    ...
    BOT_DIR="/var/www/html/zerobotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\033[31m[ERROR]\033[0m zero Bot is not installed ($BOT_DIR not found)."
    ...
    if check_marzban_installed; then
        ...
        BACKUP_SCRIPT="/root/backup_zero_marzban.sh"
        MYSQL_CONTAINER=$(docker ps -q --filter "name=mysql" --no-trunc)
        if [ -z "$MYSQL_CONTAINER" ]; then
...
EOF
        ...
        BACKUP_SCRIPT="/root/zero_backup.sh"
...
function manage_additional_bots() {
    # Check if zero main bot is installed
    if [ ! -d "/var/www/html/zerobotconfig" ]; then
        echo -e "\033[31m[ERROR]\033[0m The main zero Bot is not installed (/var/www/html/zerobotconfig not found)."
...
function change_domain() {
    ...
    CONFIG_FILE="/var/www/html/zerobotconfig/config.php"
    if [ -f "$CONFIG_FILE" ]; then
        sudo cp "$CONFIG_FILE" "$CONFIG_FILE.$(date +%s).bak"

        sudo sed -i "s/\$domainhosts = '.*\/zerobotconfig';/\$domainhosts = '${new_domain}\/zerobotconfig';/" "$CONFIG_FILE"
        ...
        curl -s -o /dev/null -F "url=https://${new_domain}/zerobotconfig/index.php" \
...
function install_additional_bot() {
    ...
    ROOT_CREDENTIALS_FILE="/root/confzero/dbrootzero.txt"
...
EOF"
    ...
    git clone https://github.com/devmrzero/botzeropanel.git "$BOT_DIR" || {
...
function update_additional_bot() {
    ...
    # List directories in /var/www/html excluding zerobotconfig
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/var/www/html/zerobotconfig" | xargs -n 1 basename)
    ...
    if ! git clone https://github.com/devmrzero/botzeropanel.git "$BOT_PATH"; then
...
function export_additional_bot_database() {
    ...
    # List all directories in /var/www/html excluding zerobotconfig
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/var/www/html/zerobotconfig" | xargs -n 1 basename)
